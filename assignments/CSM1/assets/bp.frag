...
in vec4 vs_frag_light_position[3]; // One per cascade
in vec3 vs_normal;
in vec2 vs_texcoord;

// Visualize cascade by color
...
// Calculate shadow value for a specific cascade
...
void main()
{
    ...
    float maxDist = 100.0; // Should match your far clip plane
    ...
    // Calculate shadow using the appropriate cascade
    ...
    // Optionally visualize which cascade is being used
    if (visualize_cascades) {
        vec3 cascade_color = getCascadeColor(cascadeIndex);
        obj_color = mix(obj_color, cascade_color, 0.5); // Blend with object color
    }
